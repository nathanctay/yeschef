...
import { createCookbook } from '../server/cookbooks'
import { COOKBOOK_COLORS } from '../lib/cookbookColors'

export const Route = createFileRoute('/cookbooks/new')({
  beforeLoad: async () => {
    await requireAuth()
  },
...
function NewCookbookPage() {
  ...
  const [coverStyle, setCoverStyle] = useState<'color' | 'image'>('color')
  const [selectedColor, setSelectedColor] = useState(COOKBOOK_COLORS[0].value)
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await createCookbook({ data: formData })
      navigate({ to: '/cookbooks/$id', params: { id: result.id } })
    } catch (err) {
  ...
  return (
    <main className="max-w-lg mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-[#1F2937] mb-6">New Cookbook</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {coverStyle === 'color' && (
          <div>
            ...
            {/* Hidden input carries the value into the FormData */}
